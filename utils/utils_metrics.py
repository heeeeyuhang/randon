...
def evaluteTop1_5(classfication, lines, metrics_out_path):#用于评估图像分类模型的性能，包括 Top-1 和 Top-5 精度、召回率和精确率
    correct_1 = 0
    correct_5 = 0
    preds   = []
    labels  = []
...
def fast_hist(a, b, n):#该函数用于计算混淆矩阵
    k = (a >= 0) & (a < n)
    return np.bincount(n * a[k].astype(int) + b[k], minlength=n ** 2).reshape(n, n)  

def per_class_Recall(hist):#计算每个类别的召回率
    return np.diag(hist) / np.maximum(hist.sum(1), 1) 

def per_class_Precision(hist):#计算每个类别的精确率
    return np.diag(hist) / np.maximum(hist.sum(0), 1) 

def adjust_axes(r, t, fig, axes):#用于调整图形轴以适应文本宽度
...
def draw_plot_func(values, name_classes, plot_title, x_label, output_path, tick_font_size = 12, plt_show = True):#用于绘制水平条形图，并根据需要调整轴以适应文本标签
    fig     = plt.gcf() 
    axes    = plt.gca()
...
def show_results(miou_out_path, hist, Recall, Precision, name_classes, tick_font_size = 12):#显示评估结果
...
def evaluteRecall(classfication, lines, metrics_out_path):#计算分类模型的召回率、精确度
    correct = 0
    total = len(lines)
