...
if __name__ == "__main__":
    ...
    #   修改成自己对应的种类的txt
    #----------------------------------------------------#
    classes_path    = 'model_data/cls_classes.txt' #存放着种类的别名
    #----------------------------------------------------#
    #   输入的图片大小
    #----------------------------------------------------#
    input_shape     = [224, 224]
    ...
    #   占用的显存较小，仅对网络进行微调
    #   Init_Epoch          模型当前开始的训练迭代，其值可以大于Freeze_Epoch，如设置：
    ...
    #   设置用到的显卡
    #------------------------------------------------------#
    ngpus_per_node  = torch.cuda.device_count()#1个
    if distributed:
    ...
    #   获取classes
    #------------------------------------------------------#
    class_names, num_classes = get_classes(classes_path)#cat,dog;2类

    if backbone not in ['vit_b_16', 'swin_transformer_tiny', 'swin_transformer_small', 'swin_transformer_base']:
        model = get_model_from_name[backbone](num_classes = num_classes, pretrained = pretrained)#则调用模型构建函数，并仅传递num_classes和pretrained参数。
    else:
        model = get_model_from_name[backbone](input_shape = input_shape, num_classes = num_classes, pretrained = pretrained)#调用模型构建函数，并传递input_shape、num_classes和pretrained参数。
    ...
    #------------------------------------------------------------------#
    if fp16:# 如果使用fp16进行训练，则导入GradScaler，否则设置scaler为None
        from torch.cuda.amp import GradScaler as GradScaler
        scaler = GradScaler()#自动计算和应用适当的梯度缩放因子，以保证在 FP16 下的数值稳定性。支持动态调整缩放因子，以适应不同的梯度大小和变化
    else:
        scaler = None

    model_train     = model.train()#训练模式
    #----------------------------#
    #   多卡同步Bn
    #----------------------------#
    ...
    if True:
        ...
        #   判断每一个世代的长度
        #---------------------------------------#
        epoch_step      = num_train // batch_size#每个批量有多少个数据
        epoch_step_val  = num_val // batch_size
        
        if epoch_step == 0 or epoch_step_val == 0:
            raise ValueError("数据集过小，无法继续进行训练，请扩充数据集。")

        train_dataset   = DataGenerator(train_lines, input_shape, True)#根据数据存储路径的列表，来进行数据增强,DataGenerator 类的一个实例
        val_dataset     = DataGenerator(val_lines, input_shape, False)#无需数据增强
        ...
        #   开始模型训练
        #---------------------------------------#
        for epoch in range(Init_Epoch, UnFreeze_Epoch):# Init_Epoch 到 UnFreeze_Epoch 的训练周期，循环epoch次，每次遍历全部的训练数据来训练
            #---------------------------------------#
            #   如果模型有冻结学习部分
            #   则解冻，并设置参数
            #---------------------------------------#
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:#冻结学习
                ...
                #---------------------------------------#
                lr_scheduler_func = get_lr_scheduler(lr_decay_type, Init_lr_fit, Min_lr_fit, UnFreeze_Epoch)
                #获取学习率调度函数 lr_scheduler_func，用于动态调整学习率
                ...
                if distributed:
                    batch_size = batch_size // ngpus_per_node
# train_dataset：数据集对象，提供数据。
# shuffle：是否在每个 epoch 开始时打乱数据。
# batch_size：每个批次的数据量。
# num_workers：用于数据加载的子进程数目。更多的子进程可以加快数据加载速度。
# pin_memory：如果设置为 True，数据将被复制到 CUDA 固定内存中，有助于加快 GPU 上的数据加载速度。
# drop_last：如果设置为 True，则会丢弃数据集最后一个不完整的批次（当数据量不能被 batch_size 整除时）。
# collate_fn：一个函数，用于将一个批次的数据拼接成一个张量。这里用的是 detection_collate 函数，通常用于处理不同大小的图像或不同数量的目标。
# sampler：数据采样器，用于定义数据加载顺序。这里用的是 train_sampler
                gen             = DataLoader(train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers, pin_memory=True,
                                        drop_last=True, collate_fn=detection_collate, sampler=train_sampler)#批量加载训练数据
                gen_val         = DataLoader(val_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers, pin_memory=True,
                                        drop_last=True, collate_fn=detection_collate, sampler=val_sampler)
